...
export const startUserDeletedConsumer = async () => {
  await consumeEvent('user_deleted_payment_service', async (data) => {
    const { userId } = data || {};

    if (!userId || !mongoose.Types.ObjectId.isValid(userId)) {
      throw new Error('Valid userId is required in user_deleted_payment_service event');
    }

    await Promise.all([
      Payment.deleteMany({ userId }),
      Refund.deleteMany({ userId }),
    ]);

    console.log(`✅ Removed payment records for deleted user: ${userId}`);
  });
};